#!/usr/bin/env python3
"""Discover frequent three-token formulas from parsed tablet TSV files."""
...
def discover_trigrams(
...
) -> tuple[Counter[tuple[str, str, str]], dict[tuple[str, str, str], Counter[TrigramRecord]]]:
    count_by_surface_trigram: Counter[tuple[str, str, str]] = Counter()
    payload_by_surface_trigram: dict[tuple[str, str, str], Counter[TrigramRecord]] = defaultdict(
        Counter
    )
    ...
    for path in sorted(out_dir.glob(file_glob)):
        ...
        for idx in range(len(rows) - 2):
            first, second, third = rows[idx], rows[idx + 1], rows[idx + 2]
            key = (first[0], second[0], third[0])
            count_by_surface_trigram[key] += 1
            payload_by_surface_trigram[key][
                TrigramRecord(
                    first_surface=first[0],
                    second_surface=second[0],
                    third_surface=third[0],
                    first_payload=first[1],
                    second_payload=second[1],
                    third_payload=third[1],
                )
            ] += 1
    return count_by_surface_trigram, payload_by_surface_trigram


def main() -> int:
    parser = argparse.ArgumentParser(description="Discover frequent three-token formulas.")
    ...
    parser.add_argument("--top", type=int, default=60, help="Top N trigrams to print")
    parser.add_argument(
        "--min-count",
        type=int,
        default=6,
        help="Only print trigrams with count >= min-count",
    )
    args = parser.parse_args()

    counts, payloads = discover_trigrams(
        out_dir=Path(args.out_dir),
        file_glob=args.file_glob,
    )

    printed = 0
    for (first_surface, second_surface, third_surface), count in counts.most_common():
        if count < args.min_count:
            continue
        variants = payloads[(first_surface, second_surface, third_surface)]
        top_record, top_count = variants.most_common(1)[0]
        top_share = float(top_count) / float(count)
        print(
            f"{first_surface} {second_surface} {third_surface}\tcount={count}\t"
        ...
        print(
            "  third:",
            top_record.third_payload.analysis,
            "|",
            top_record.third_payload.dulat,
            "|",
            top_record.third_payload.pos,
            "|",
            top_record.third_payload.gloss,
        )
        printed += 1
        if printed >= args.top:
            break
