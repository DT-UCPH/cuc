"""Compare legacy and spaCy-based `k`-context strategies on the tablet corpus."""
...
def main() -> int:
    ...
    args = _parse_args()
    with tempfile.TemporaryDirectory(prefix="compare-k-context-") as tmp_dir:
        ...
        _apply_steps(
            [pre_dir / name for name in target_names], list(pre_pipeline.pre_l_context_steps)
        )
        _apply_steps([pre_dir / name for name in target_names], list(pre_pipeline.l_context_steps))
        _apply_steps(
            [pre_dir / name for name in target_names], list(pre_pipeline.pre_k_context_steps)
        )
        ...
        _apply_steps(legacy_targets, build_legacy_k_context_steps())
        _apply_steps(legacy_targets, list(legacy_pipeline.post_k_context_steps))
        _apply_steps(spacy_targets, list(spacy_pipeline.k_context_steps))
        _apply_steps(spacy_targets, list(spacy_pipeline.post_k_context_steps))
