"""Compare legacy and spaCy-based offering-context strategies on the tablet corpus."""
...
def main() -> int:
    ...
    args = _parse_args()
    with tempfile.TemporaryDirectory(prefix="compare-offering-context-") as tmp_dir:
        ...
        _apply_steps(
            ...
            list(pre_pipeline.pre_offering_context_steps),
        )
        ...
        _apply_steps(legacy_targets, build_legacy_offering_context_steps())
        _apply_steps(legacy_targets, list(legacy_pipeline.post_offering_context_steps))
        _apply_steps(spacy_targets, list(spacy_pipeline.offering_context_steps))
        _apply_steps(spacy_targets, list(spacy_pipeline.post_offering_context_steps))
