"""Compare legacy and spaCy-based formula-context strategies on the tablet corpus."""
...
def main() -> int:
    ...
    args = _parse_args()
    with tempfile.TemporaryDirectory(prefix="compare-formula-context-") as tmp_dir:
        ...
        _apply_steps(
            ...
            list(pre_pipeline.pre_formula_context_steps),
        )
        ...
        _apply_steps(legacy_targets, build_legacy_formula_context_steps())
        _apply_steps(legacy_targets, list(legacy_pipeline.post_formula_context_steps))
        _apply_steps(spacy_targets, list(spacy_pipeline.formula_context_steps))
        _apply_steps(spacy_targets, list(spacy_pipeline.post_formula_context_steps))
