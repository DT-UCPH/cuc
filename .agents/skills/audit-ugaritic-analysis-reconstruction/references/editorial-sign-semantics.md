# KTU Editorial Sign Semantics

Treat the sign span as editorial evidence, not as morphological analysis syntax. Keep four
layers distinct:

1. **Sign span:** KTU markup preserving physical and editorial information.
2. **Edited reading:** the linguistic word after the sign-level operations are applied.
3. **Lexical lookup:** the edited reading used to find DULAT candidates.
4. **Analysis:** the selected lexeme aligned against the edited reading.

## Safe Automatic Treatment

| KTU notation | Meaning | Automatic treatment |
| --- | --- | --- |
| `[[x]]` | erased sign(s), line(s), or correction | Remove `x` from the edited reading and morphology target. |
| `{x}` | redundant sign | Remove `x` from the edited reading and morphology target. |
| `<x>` | missing sign supplied editorially | Keep `x` in the normalized reading; do not create a corrected alias merely because of the brackets. |
| `[x]`, `[`, `]` | restored damaged passage or break boundary | Keep readable/restored letters in the normalized reading; delimiters do not create morphology. |
| `+`, `(+)` | physical or indirect fragment join | Treat as layout evidence; do not create a lexical alias. |
| `(...)` | special editorial remark | Do not infer lexical content automatically. |
| `\` | line continued by editors | Treat as layout evidence; do not create a lexical alias. |
| italic continuation marker | line continued by the scribe | Treat as editorial/layout evidence unless TF exposes an independently validated letter reading. |
| `xx . xx`, `x.x` | word divider | Preserve an internal divider in the sign span when it occurs inside a TF token, but do not use it to invent an alias. |

Only `[[...]]` and `{...}` support automatic edited-reading removal, and only after all
letters in the sign span normalize exactly to the TF physical surface. Reject mismatching,
ambiguous, empty, one-letter, parenthetical, or divider-crossing corrected aliases.

## Align After Lexical Selection

Do not translate `[[x]]` or `{x}` to `&x` or `(x`. Apply the sign-level operation first, use
the edited reading to retrieve linguistically viable DULAT candidates, select among homonyms
with textual and lexical evidence, then align the selected lexeme against the edited reading.
An `&` is legitimate only for material that survives in that edited word but lies outside the
selected lexical representation.

Examples:

- `g[[m]]pn`: physical `gmpn`, edited reading `gpn`, selected `gpn (III)`, analysis
  `gpn(III)/`.
- `aṯ{t}rt`: physical `aṯtrt`, edited reading `aṯrt`, selected `ảṯrt (II)`, analysis
  `aṯr(t(II)/t`.
- `<n>ḫtu`: physical and lookup reading `nḫtu`; DULAT morphology identifies an N-stem suffix
  form, yielding `]n]ḫt(ʔ[&u`. The angle brackets themselves do not imply `]n]`.

Validate the final analysis against the edited reading, not the pre-edit physical token and
not the raw sign-span punctuation. Preserve the full sign span in reviewed TSVs.

## Comment and Source Discipline

- Do not add a comment that merely restates editorial notation already visible in the sign
  span, such as “restored in lacuna,” “erased,” “redundant,” or “broken context.”
- Do not label a reading as `KTU` unless it was checked directly against a verified KTU
  edition. CUC, older CUC exports, TF features, UDB, and database fields named `KTU` are not
  the original KTU edition and must be identified by their actual provenance when cited.
- Add a comment only when it preserves non-obvious linguistic or textual evidence, a verified
  source attribution, or an explicit reviewer decision not already encoded by the sign span.
